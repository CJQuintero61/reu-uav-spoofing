...
This module contains the classes for making charts for chart_maker.py.
"""
import os
import numpy as np
...
class GroupStats:
    ...
    def plot_size_chart(self):
        ...
        tallest_label = max(m + s for m, s in zip(means, stds)) + 0.12
        ax.set_ylim(0, tallest_label)
        ax.set_title(f'Model Size Comparison (Mean ± Std)')
        ax.grid(axis='y', linestyle='--', alpha=0.4)

        plt.tight_layout()
        save_path = os.path.join(CHARTS_DIR, f'Model_Size_Comparison.png')
        plt.savefig(save_path, dpi=200)